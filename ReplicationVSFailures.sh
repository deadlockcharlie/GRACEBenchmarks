...
RESULTS_DIRECTORY="$ROOT_DIRECTORY/Results/ReplicationVSFailures"
...
bin/ycsb.sh run grace -s -P workloads/workload_grace -p  HOSTURI="http://localhost:3000" -p DBTYPE="memgraph" -p DBURI="bolt://localhost:7687" -p maxexecutiontime=60 -p threadcount=1 > $RESULTS_DIRECTORY/GRACE/results.txt
