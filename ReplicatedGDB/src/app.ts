...
logger.info(`Connecting to Y.js provider at: ${wsUri}`);

export const yjsProvider = new WebsocketProvider(
  wsUri,
  "GraceSyncKey",
  ydoc
);
...
yjsProvider.on('status', (event: { status: string }) => {
  logger.info(`Y.js WebSocket status: ${event.status}`);
});

yjsProvider.on('connection-close', (event: any) => {
  logger.warn(`Y.js WebSocket connection closed: ${JSON.stringify(event)}`);
});

yjsProvider.on('connection-error', (error: any) => {
  logger.error(`Y.js WebSocket connection error: ${JSON.stringify(error)}`);
  if (error && error.error) {
    logger.error(`Error details: ${error.error.message || error.error}`);
  }
});
...
logger.info(`DATABASE_URI: ${process.env.DATABASE_URI}`);
logger.info(`WS_URI: ${process.env.WS_URI}`);
logger.info(`LOG_LEVEL: ${process.env.LOG_LEVEL}`);
logger.info(`DC_ID: ${process.env.DC_ID}`);
...

{
  ...
  app.get('/health', (req, res) => {
    res.json({
      preloadDone,
      yjsConnected: yjsProvider.wsconnected,
      yjsSynced: yjsProvider.synced,
